...
import authService from "../../services/authService"; // Import authService to get the current user
import {
  Container,
  Typography,
  TextField,
  ...
  useEffect(() => {
    const fetchManagers = async () => {
      try {
        ...
        setManagers(response.data.filter((user) => user.roleid === 2)); 
        ...
        try {
          ...
          setProject({
            ...
            userids: projectData.users
              ? projectData.users.map((user) => user.userid)
              : [],
          });
        } catch (error) {
